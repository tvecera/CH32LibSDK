...
{
	DrawRect(((WIDTHM-NETW)/2)>>COORD_SHIFT, 0, NETW>>COORD_SHIFT, height>>COORD_SHIFT, COL_GRAY);
}
...
void DispPaddle(int inx)
{
	...
	DrawRect(x, (y - PADDLEH/2)>>COORD_SHIFT, PADDLEW>>COORD_SHIFT, PADDLEH>>COORD_SHIFT, COL_GRAY);
	OldPaddleY[inx] = y;
}
...
void OpenDraw()
{
	...
	// draw menu
	PrintTextAt("Press key to start:", 0, 5, COL_GRAY);

	PrintTextAt("LEFT", 0, 7, COL_GRAY);
	PrintTextAt("Left paddle", 6, 7, COL_WHITE);

	PrintTextAt("RIGHT", 0, 8, COL_GRAY);
	PrintTextAt("Right paddle", 6, 8, COL_WHITE);

	PrintTextAt("DOWN", 0, 9, COL_GRAY);
	PrintTextAt("2 players", 6, 9, COL_WHITE);

	PrintTextAt("UP", 0, 10, COL_GRAY);
	PrintTextAt("Demo", 6, 10, COL_WHITE);

	PrintTextAt("A", 0, 11, COL_GRAY);
