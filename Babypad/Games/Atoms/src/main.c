...
#define MENUY 10
		...
		PrintTextAt("Exploding Atoms", (TEXTWIDTH - 15)/2, MENUY - 3, COL_WHITE);

		PrintTextAt("Setup players:", MENUX, MENUY, COL_WHITE);

		PrintTextAt("LEFT ..... Player 1 is", MENUX, MENUY+2, COL_WHITE);
		PrintTextAt("RIGHT .... Player 2 is", MENUX, MENUY+3, COL_WHITE);

		PrintTextAt("A ........ Start The Game", MENUX, MENUY+7, COL_WHITE);

		for (i = 0; i < PLAYER_NUM; i++)
			PrintTextAt(PlayerTypeTxt[Players[i].type], MENUX+23, MENUY+(i+2), COL_WHITE);
...
void DispScore(u8 player)
{
	...
	{
		PrintTextAt("-lost-", INFOX + 2, Players[player].scorey + 2, COL_WHITE);
	}

	// player is winner
	else if (PlayerNum == 1)
	{
		PrintTextAt("WINNER", INFOX + 2, Players[player].scorey + 2, COL_WHITE);
	}
	else
	{
		...
		// display score
		PrintTextAt(DecNumBuf, INFOX + 4, Players[player].scorey + 2, COL_WHITE);
		if (len == 1) PrintCharAt(' ', INFOX + 5, Players[player].scorey + 2, COL_WHITE);
		if ((len == 1) || (len == 2)) PrintCharAt(' ', INFOX + 6, Players[player].scorey + 2, COL_WHITE);
	}
}
...
//		if (Players[i].type != PLAYER_OFF) // player is in game
		{
			...
			PlayerText[7] = i + '1';
			PrintTextAt(PlayerText, x+1, y, COL_WHITE);
			...
			if (i == Player)
			{
				PrintCharAt(28, x, y, COL_WHITE);
				for (j = 1; j < 9; j++) PrintCharAt(21, x+j, y, COL_WHITE);
				PrintCharAt(25, x+9, y, COL_WHITE);

				PrintCharAt(26, x, y+1, COL_WHITE);
				PrintCharAt(26, x+9, y+1, COL_WHITE);

				PrintCharAt(26, x, y+2, COL_WHITE);
				PrintCharAt(26, x+9, y+2, COL_WHITE);

				PrintCharAt(26, x, y+3, COL_WHITE);
				PrintCharAt(26, x+9, y+3, COL_WHITE);

				PrintCharAt(22, x, y+4, COL_WHITE);
				for (j = 1; j < 9; j++) PrintCharAt(21, x+j, y+4, COL_WHITE);
				PrintCharAt(19, x+9, y+4, COL_WHITE);
			}
			else
			{
				for (j = 0; j < 10; j++) PrintCharAt(' ', x+j, y, COL_WHITE);

				PrintCharAt(' ', x, y+1, COL_WHITE);
				PrintCharAt(' ', x+9, y+1, COL_WHITE);

				PrintCharAt(' ', x, y+2, COL_WHITE);
				PrintCharAt(' ', x+9, y+2, COL_WHITE);

				PrintCharAt(' ', x, y+3, COL_WHITE);
				PrintCharAt(' ', x+9, y+3, COL_WHITE);

				for (j = 0; j < 10; j++) PrintCharAt(' ', x+j, y+4, COL_WHITE);
			}
		}
	}
}
...
void DispGrid()
{
	...
	for (y = 0; y < BOARDH-2; y += 3)
	{
		...
		{
			PrintCharAt(26, x+2, y, COL_GRAY);
			PrintCharAt(26, x+2, y+1, COL_GRAY);
			PrintCharAt(21, x, y+2, COL_GRAY);
			PrintCharAt(21, x+1, y+2, COL_GRAY);
			PrintCharAt(31, x+2, y+2, COL_GRAY);
		}
		PrintCharAt(21, x, y+2, COL_GRAY);
		PrintCharAt(21, x+1, y+2, COL_GRAY);
	}
...
void DispTile(u8 inx, Bool cursor)
{
	...
	// display tile
	PrintCharAt((n > 0) ? stone : empty, x, y, COL_WHITE);
	PrintCharAt((n > 1) ? stone : empty, x+1, y, COL_WHITE);
	PrintCharAt((n > 2) ? stone : empty, x, y+1, COL_WHITE);
	PrintCharAt((n > 3) ? stone : empty, x+1, y+1, COL_WHITE);
}
