...
#define FILEROWS	(TEXTHEIGHT-1) // rows of files (=9)
...
void SysSetup()
{
	...
	// battery input from PB0 (A8)
	RCC_PBClkEnable();
	GPIO_Mode(PB0, GPIO_MODE_AIN);
	...
	u32 vref = ADC1_GetSingleMul(15, B14) >> 7; // result value (12+14-7) 19 bits
	vref = (u32)(1200 << 19) / vref;

	// re-initialize ADC single conversion mode
	ADC1_InitSingle();

	// measure battery voltage (input A8)
	u32 vbat = ADC1_GetSingleMul(8, B14) >> 7; // result value (12+14-7) 19 bits
	vbat = (vbat * vref) >> 19;
	...
	PrintText("\26 Supply:\27 ");
	PrintText(DecNumBuf);
	PrintText("mV\n");

	// display battery voltage
	DecUNum(DecNumBuf, vbat, 0);
	PrintText("\26Battery:\27 ");
	PrintText(DecNumBuf);
	PrintText("mV\n");

	// display display level
	DecUNum(DecNumBuf, DispBacklightGet(), 0);
	PrintText("\26Display:\27 ");
	PrintText(DecNumBuf);
	PrintText("\n");

	PrintText("\n\n\22LEFT/RIGHT=display");
	PrintText("\nB=Up-Dir\nY=Back");
