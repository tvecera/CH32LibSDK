...
#define APPINFO_MCU	0x5833		// this MCU serie identifier in hex format (= "3X")
#define APPINFO_MAGIC2	0x6F666E69	// magic 2 "info" in hex format

#define LOADER_SIZE		0x5000					// loader size
#define APPINFO_OFF		(LOADER_SIZE + 0xDC)	// application info header offset
...
typedef struct {
	...
	char	mcu[2];		// 0x0A: MCU serie identifier ("3X" = CH32X03x)
						//		"03" = CH32V003
						//		"0X" = CH32V00x
						//		"3X" = CH32X03x
						//		"V1" = CH32V103
...
#define CRC16C_RES2 0x9144
	...
	if ((*(u32*)app->magic1 != APPINFO_MAGIC1) // magic "INFO"
		|| (app->len != 0x01234567) // application length
		|| (app->crc != 0x89AB) // application CRC
		|| (*(u16*)app->mcu != APPINFO_MCU) // MCU identifier
		|| (*(u32*)app->magic2 != APPINFO_MAGIC2)) // magic "info"
	{
