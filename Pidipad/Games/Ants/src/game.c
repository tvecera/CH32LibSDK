...
void DispChar(u8 ch)
{
	PrintCharAt(ch+(DispCol & B7), DispPos, DispRow, DispCol & 0x7f);
	DispPos++;
}
...
#define COL_DISABLE	COL_GRAY

		const sCard* card = &Card[type];
		u8 col = (card->type == PAR_BRICKS) ? COL_RED : ((card->type == PAR_WEAPONS) ? COL_GREEN : COL_CYAN);
		if (disable) col = COL_DISABLE;
		...
		DispPos = x;
		DispChar(133);
		DispCol = disable ? COL_DISABLE : COL_YELLOW;
		...
		else
		{
			...
			DispCol = disable ? COL_DISABLE : COL_WHITE;
			DispText(card->text1);
			DispCharRep(' ', x + CARDW-1 - DispPos);
		}
		DispCol = col;
...
#define CHAR_FULL	1
		...
		DispCol = ((i & 1) == 0) ? COL_WHITE : COL_BLUE;
		ch = (h > 1) ? CHAR_FULL : ' ';
		DispCharRep(ch, 2);
...
void DispTitle()
{
	...
	{
		// player color
		u8 colframe = (Player == player) ? COL_BLUE : COL_GRAY;
		u8 coltext = (Player == player) ? COL_RED : COL_GRAY;
...
void SaveCard(int x, int y)
{
	...
	// save attributes
	src = &AttrBuf[x/2 + y*ATTRWIDTHBYTE];
	for (i = CARDH; i > 0; i--)
	{
		memcpy(dst, src, CARDW/2);
		dst += CARDW/2;
		src += ATTRWIDTHBYTE;
	}
}
...
void LoadCard(int x, int y)
{
	...
	// load attributes
	dst = &AttrBuf[x/2 + y*ATTRWIDTHBYTE];
	for (i = CARDH; i > 0; i--)
	{
		memcpy(dst, src, CARDW/2);
		src += CARDW/2;
