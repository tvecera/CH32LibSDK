...
{
	DrawRect(((WIDTHM-NETW)/2)>>COORD_SHIFT, 0, NETW>>COORD_SHIFT, height>>COORD_SHIFT, COL_CYAN);
}
...
void DispScore(int inx)
{
	...
	// decode digits
	DrawImgBg(ImgDigits + dig[0], x, 0, 8, 10, 10, COL_GREEN);
	DrawImgBg(ImgDigits + dig[1], x+8, 0, 8, 10, 10, COL_GREEN);
}
...
void DispBall()
{
	if (BallState == BALL_IN)
	{
		...
		DrawRect((x - BALLSIZE/2)>>COORD_SHIFT, (y - BALLSIZE/2)>>COORD_SHIFT,
			BALLSIZE>>COORD_SHIFT, BALLSIZE>>COORD_SHIFT, COL_RED);
		OldBallX = x;
		OldBallY = y;
	}
}
...
void OpenDraw()
{
	...
	// title
	PrintTextAt("TV Tennis", (TEXTWIDTH-9)/2, 2, COL_YELLOW);
