...
void Psw()
{
	...
	for (;;)
	{
		...
		{
			buf2[0] = buf[pos];
			InfoDispText(x + pos*6, buf2, COL_YELLOW | COL_INV);
		}
...
//void DrawImgFast(const u8* img, const u8* attr, int x, int y, int xs, int ys, int w, int h, int wsb, int wsab);
	DrawImgFast(ImgTiles, ImgTiles_Attr, x*TILESIZE, y*TILESIZE + MAPY,
		TileXY[tile*2], TileXY[tile*2+1], TILESIZE, TILESIZE, 20, 40);
}
...
void CongLevel()
{
	...
	// display text
	DrawTextCond6(CongTxt, (WIDTH - (sizeof(CongTxt)-1)*6)/2, BOXY + 8, COL_YELLOW);
	DrawTextCond6(Cong2Txt, (WIDTH - (sizeof(Cong2Txt)-1)*6)/2, BOXY + 2*8, COL_GREEN);
	DrawTextCond6(Cong3Txt, (WIDTH - (sizeof(Cong3Txt)-1)*6)/2, BOXY + 3*8, COL_GREEN);
...
void OpenLevel()
{
	...
	DrawTextCond6(LevelTxt, x, BOXY + 8, COL_GREEN);
	x += sizeof(LevelTxt)*6;
	DrawTextCond6(buf, x, BOXY + 8, COL_YELLOW);

	// display password
	x = (WIDTH - (sizeof(PswTxt)+PSWLEN)*6)/2;
	DrawTextCond6(PswTxt, x, BOXY + 3*8, COL_GREEN);
	memcpy(buf, &LevelPsw[Level*PSWLEN], PSWLEN);
	buf[PSWLEN] = 0;
	x += sizeof(PswTxt)*6;
	DrawTextCond6(buf, x, BOXY + 3*8, COL_YELLOW);
