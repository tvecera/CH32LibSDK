...
#define YP5 sprite[4].Ypos
	...
	{ 
		...
		for (x = 0; x < 128; x++)
		{
			...
			DispDirectDraw(blitzSprite(XP1,YP1,x,y,Player1_Rotate,PLAYER_WHITE_B),x,y,Ply);
			DispDirectDraw(blitzSprite(XP2,YP2,x,y,Computer_Rotate,PLAYER_BLACK_B) ,x,y,Cpu);
			DispDirectDraw(blitzSprite(XP3,YP3,x,y,0,puck) ,x,y,COL_GREEN); //color(31,63,0));

			if ((y >= 2) && (y < 6))
			{
				if ((x >= 117) && (x < 124))
					DispDirectDraw(blitzSprite(XP5,YP5,x,y,0,GOALER_BLACK) ,x,y,Cpu);
				else if ((x >= 4) && (x < 9))
					DispDirectDraw(blitzSprite(XP4,YP4,x,y,0,GOALER_WHITE) ,x,y,Ply);
			}
		}
	}
}
	...
	for (y = 0; y < 8; y++)
	{
		...
		for (x = 0; x < 128; x++)
		{
			...
			case 1:
				...
				DispDirectDraw( blitzSprite(40+(13),39+SLIDE+(13),x,y,Points_Pl1,NUM),x,y,Ply);
				DispDirectDraw( blitzSprite(40+(33),39+SLIDE+(13),x,y,Points_CPU,NUM),x,y,Cpu);
				break;
