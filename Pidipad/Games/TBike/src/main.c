...
NEW_START:
	...
	while(1)
	{
		...
		if (PLANTAGE_0==0)
		{
			if (TRIG_OK==0)
			{
				if ((TINYJOYPAD_LEFT)&&(ACCEL>1))
				{
					BypassWheelupreset=1;
					if ((Wheel_up<5) && (Latch1==0)) Wheel_up++;
				}
				else if ((TINYJOYPAD_RIGHT) && (ACCEL>1))
				{
					BypassWheelupreset=1;
...
void analise_minutieuse(void)
{
	...
	case(3):
		if ((ACCEL>2) && (FREEAIR==0)) ACCEL=ACCEL-0.20;
		break;
	...
	case(8):
		if (FREEAIR==0) ADD_LIVE(NoSprite);
		break;
