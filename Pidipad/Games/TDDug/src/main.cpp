...
#define GAME_FRAME_CONTROL_TDDUG FPS_Control(50) //76)
//while((currentMillis-MemMillis)<76){currentMillis=millis();}MemMillis=currentMillis
...
//unsigned long currentMillis_TB;
//unsigned long MemMillis_TB;
u32 MemMillis;

void FPS_Control(uint8_t Ms_)
{
			...
			if (W_TDDUG[t2].ACTIVE()&&(ENEMY_DDUG[t3].ACTIVE()))
			{
				if (Universal_TDDUG(ex_,ey_,ew_,eh_,bx_,by_,bh_,bw_))
				{
					...
					if (ENEMY_DDUG[t3].FIRST())
					{
						...
						if (ONE_ADD==0)
						{
							ENEMY_DDUG[t3].PUT_BIG_ZIP((ENEMY_DDUG[t3].BIG_ZIP()<30) ?
								ENEMY_DDUG[t3].BIG_ZIP()+1 : ENEMY_DDUG[t3].BIG_ZIP());
							ONE_ADD++;
						}
					}
				}
...
uint8_t blitzSprite_TDDUG(int8_t xPos,int8_t yPos,uint8_t xPASS,uint8_t yPASS,uint8_t FRAME,const uint8_t *SPRITES)
{
	...
	{
		OUTBYTE=SplitSpriteDecalageY_TDDUG(SPRITEyDECALAGE,SPRITES[ScanA+(PICBYTE)],1);
	}
	if ((SPRITEyLINE>0))
	{
