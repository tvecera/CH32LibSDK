...
#define EMPTY_COL	COL_RED		// empty color
...
void DrawBoard()
{
	...
	for (i = 0; i < TILESYNUM; i++)
	{
		for (j = 0; j < TILESXNUM; j++)
		{
			...
			if (b == TILE_EMPTY)
			{
				PrintCharAt(1, x, y, EMPTY_COL);
				PrintCharAt(1, x+1, y, EMPTY_COL);
				PrintCharAt(1, x+2, y, EMPTY_COL);
				PrintCharAt(1, x+3, y, EMPTY_COL);

				PrintCharAt(1, x, y+1, EMPTY_COL);
				PrintCharAt(1, x+1, y+1, EMPTY_COL);
				PrintCharAt(1, x+2, y+1, EMPTY_COL);
				PrintCharAt(1, x+3, y+1, EMPTY_COL);

				PrintCharAt(1, x, y+2, EMPTY_COL);
				PrintCharAt(1, x+1, y+2, EMPTY_COL);
				PrintCharAt(1, x+2, y+2, EMPTY_COL);
				PrintCharAt(1, x+3, y+2, EMPTY_COL);
			}
			else
			{
				PrintCharAt(156, x, y, TILE_COL);
				PrintCharAt(149, x+1, y, TILE_COL);
				PrintCharAt(149, x+2, y, TILE_COL);
				PrintCharAt(153, x+3, y, TILE_COL);

				PrintCharAt(154, x, y+1, TILE_COL);
				PrintCharAt((b<9) ? (' '+128) : ('1'+128), x+1, y+1, TILE_COL);
				PrintCharAt((((b+1) % 10)+'0'+128), x+2, y+1, TILE_COL);
				PrintCharAt(154, x+3, y+1, TILE_COL);

				PrintCharAt(150, x, y+2, TILE_COL);
				PrintCharAt(149, x+1, y+2, TILE_COL);
				PrintCharAt(149, x+2, y+2, TILE_COL);
				PrintCharAt(147, x+3, y+2, TILE_COL);
			}
		}
	}
}
...
void WaitOn()
{
	...
	PrintCharAt(8, x+1, y+1, EMPTY_COL);
}
...
void WaitOn2()
{
	int x = HoleX()*TILEW + TILEX;
	int y = HoleY()*TILEH + TILEY;
	PrintCharAt(9, x+1, y+1, EMPTY_COL);
}
...
Restart:
		// draw title
		DrawText2("FIFTEEN", (160-14*8)/2, 0, COL_YELLOW);
		PrintTextAt("Press a key to start", 0, 14, COL_CYAN);
