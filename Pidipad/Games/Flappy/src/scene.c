...
int SceneSetNum = 105;	// number of levels in current scene set
...
void ParseScene()
{
	...
	// current scene
	const u8* s = SharpSceneList1[SceneInx];
	Scene = s;
...
// --- show screen with scene number
	...
	// clear header
	DrawRectClr(BOARDX+12*STEP, BOARDY+STEP, 14*STEP, 2*STEP);
	DrawRectClr(BOARDX+27*STEP, BOARDY+STEP, 12*STEP, 2*STEP);
