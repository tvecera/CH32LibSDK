...
const sTile Tiles[TILES_NUM] = {
	{ 13, 10, 1, 1 },	// 0: empty tile
	{ 20, 14, 1, 1 },	// 1: red wall
	{ 21, 14, 1, 1 },	// 2: blue wall
	...
	{ 22, 8, 1, 1 },	// 82: digit 5
	{ 20, 6, 1, 1 },	// 83: digit 6
	{ 22, 10, 1, 1 },	// 84: digit 7
	{ 20, 8, 1, 1 },	// 85: digit 8
	{ 22, 12, 1, 1 },	// 86: digit 9
...
{
	if (tile == SPACETILE)
		DrawRectClr(x*STEP+MAPX, y*STEP+MAPY, STEP, STEP);
	else
	{
		const sTile* t = &Tiles[tile];
		DrawImgBg(ImgTilesRed + t->x/2 + t->y*STEP*8, x*STEP+MAPX, y*STEP+MAPY, t->w*STEP, t->h*STEP, 8, COL_RED);
		DrawImg(ImgTilesYellow + t->x/2 + t->y*STEP*8, x*STEP+MAPX, y*STEP+MAPY, t->w*STEP, t->h*STEP, 8, COL_YELLOW);
		DrawImg(ImgTilesBlue + t->x/2 + t->y*STEP*8, x*STEP+MAPX, y*STEP+MAPY, t->w*STEP, t->h*STEP, 8, COL_BLUE);
	}
}
...
{
	const sTile* t = &Tiles[tile];
	DrawRectClr(x*STEP+MAPX, y*STEP+MAPY, t->w*STEP, t->h*STEP);
}
