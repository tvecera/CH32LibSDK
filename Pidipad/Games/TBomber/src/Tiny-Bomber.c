...
void FPS_Control_TB(uint8_t Ms_)
{
	u32 ms = (u32)Ms_ * HCLK_PER_MS;
	u32 start = MemMillis_TB;
	u32 t;
...
void FPS_Control_TB(uint8_t Ms_)
{
	while ((currentMillis_TB - MemMillis_TB) < Ms_)
	{
		currentMillis_TB = pico_millis();
	}
	MemMillis_TB = currentMillis_TB;
}
*/
// fin var public
...
void LoadLevel(uint8_t LeveL)
{
	...
	for (t=0; t<14; t++)
	{
		BlocBombMem[t]=level[t+(LeveL*14)];
	}
}
	...
	while(1)
	{
		...
		if (Frame%2==0)
		{
			...
			if (INGAME==1)
			{
				...
				for (t=0; t<44; t=t+2)
				{
					Sound(Music_bomber[t],(Music_bomber[t+1]));//+60); !!!!!!!!!!!!!!!!!!!!!
				}
...
#define CHECKCOLLISION ((RECUPE)&(TempPGMByte))
		...
		if (Sprite[SpriteCheck].DirectionH==1)
		{
			...
			for(t=0; t<=6; t++)
			{
				...
				{
					TempPGMByte=back[((Sprite[SpriteCheck].y+tadd)*128) +
...
#define YPOSFIRE (BOMBXY[1]-1)
	if (BOMBXY[2]==1)
	{
		if ((x>7) && (y>0))
		{
			...
			{
				return fire[((x-(XPOSFIRE))+((y-(YPOSFIRE))*72))+(BOMBXY[3]*24)];
			}
		}
	}
	return 0x00;
...
uint8_t BombBlitz(uint8_t x,uint8_t y)
{
	if ((x>=BOMBXY[0]) && (x<=BOMBXY[0]+7) && (y==BOMBXY[1]))
		return bomb[(x-BOMBXY[0])+(BOMBXY[3]*8)];
	else
		return 0x00;
}
...
uint8_t return_if_sprite_present(uint8_t x,struct PERSONAGE  *Sprite,uint8_t SpriteNumber)
{
	if ((x>=Sprite[SpriteNumber].x) && (x<(Sprite[SpriteNumber].x+8)))
	{
		...
		return caracters[((x-Sprite[SpriteNumber].x) + (8*(Sprite[SpriteNumber].type*12)))
