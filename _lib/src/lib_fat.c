...
#if USE_FAT		// 1=use FAT filesystem
...
Bool Disk_SyncFS()
{
	...
	if ((DiskFS == FS_FAT32) && (FSinfo == FSI_DIRTY))
	{
		...
		SD_WriteSect(DiskBufSect, DiskBuf);
...
Bool Disk_DirClear(u32 clust)
{
	...
	for (; num > 0; num--)
	{
		if (!SD_WriteSect(sect, DiskBuf)) return False;
		sect++;
	}
...
DISKFORMERR:
	...
	if (mbr)
	{
		...
		// write sector to disk
		if (!SD_WriteSect(0, DiskBuf)) goto DISKFORMERR;
	...
	if (!SD_WriteSect(hsect, DiskBuf)) goto DISKFORMERR;

	// create FSINFO sector
	if (fs2 == FS_FAT32)
	{
		// write backup boot sector to disk
		if (!SD_WriteSect(hsect + 6, DiskBuf)) goto DISKFORMERR;
		...
		if (!SD_WriteSect(hsect + 1, DiskBuf)) goto DISKFORMERR;

		// write backup FSINFO sector to disk
		if (!SD_WriteSect(hsect + 7, DiskBuf)) goto DISKFORMERR;
	}
	...
	if (mbr)
	{
		...
		if (!SD_ReadSect(0, DiskBuf)) goto DISKFORMERR;
...
Bool FileCheckExt(sFileInfo* fi, const char* ext)
{
	...
	for (; len > 0; len--)
	{
		s--;
		d--;
		if (FileUpperCase(*s) != FileUpperCase(*d)) return False;
	}
