...
*/
	...
	{
		col = ((col >> 1) == 0) ? 0 : 0xf0;
		a[0*ATTRWIDTHBYTE] = (a[0*ATTRWIDTHBYTE] & 0x0f) | col;
		a[1*ATTRWIDTHBYTE] = (a[1*ATTRWIDTHBYTE] & 0x0f) | col;
		a[2*ATTRWIDTHBYTE] = (a[2*ATTRWIDTHBYTE] & 0x0f) | col;
		a[3*ATTRWIDTHBYTE] = (a[3*ATTRWIDTHBYTE] & 0x0f) | col;
	}
	else
	{
		col = ((col >> 1) == 0) ? 0 : 0x0f;
		a[0*ATTRWIDTHBYTE] = (a[0*ATTRWIDTHBYTE] & 0xf0) | col;
		a[1*ATTRWIDTHBYTE] = (a[1*ATTRWIDTHBYTE] & 0xf0) | col;
		a[2*ATTRWIDTHBYTE] = (a[2*ATTRWIDTHBYTE] & 0xf0) | col;
		a[3*ATTRWIDTHBYTE] = (a[3*ATTRWIDTHBYTE] & 0xf0) | col;
	}
