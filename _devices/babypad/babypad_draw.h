...
#ifndef _BABYPAD_DRAW_H
#define _BABYPAD_DRAW_H
...
// Colors
// Three colors (including gray) are only displayed in video modes that
// support gray. In other video modes, white is displayed instead of gray.
// Bit 0 of the color is written to the FrameBuf frame buffer as a white
// pixel. Bit 1 of the color is written to the AttrBuf attribute buffer
// and indicates the higher or lower brightness of the white pixel.
#define COL_BLACK	0		// black color
#define COL_GRAY	B0		// The gray color is only displayed in video modes
					// that support gray. In other modes, it is displayed as white.
...
#if (VMODE == 1) || (VMODE == 2) // only graphics mode
...
#endif // (VMODE == 1) || (VMODE == 2) // only graphics mode
#endif // USE_DRAW
