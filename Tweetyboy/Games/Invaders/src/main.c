...
#define LINEY2		(HEIGHT-9) // line 2 Y coordinate
#define LEVELY		(HEIGHT-7) // level Y coordinate
#define LEVELX		(WIDTH-6*8) // level X coordinate
#define SCOREY		(HEIGHT-7) // score Y coordinate
#define SCOREX		(LEVELX-9*8-4) // score X coordinate
...
#define SHIPY		(HEIGHT-10-SHIPH) // ship Y coordinate
...
#define HOUSEDX		28	// house delta X coordinate
...
#define NEXTMOTHER	350	// number of steps to generate new mothership
...
#define ALIENNUMY	4	// number of aliens in Y direction
#define ALIENNUM	(ALIENNUMX*ALIENNUMY) // number of aliens
#define ALIENSPACEX	2	// space between aliens in X direction
...
#define ALIENMAXY	15	// alien block initial max. Y coordinate
...
const u8 AlienType[ALIENNUMY] = { 0, 1, 1, 2 }; // types of aliens
const u8 AlienColor[ALIENNUMY] = { ALIEN1_COL, ALIEN2_COL, ALIEN2_COL, ALIEN3_COL };
const u8 AlienScore[ALIENNUMY] = { 30, 20, 20, 10 }; // score of aliens
...
{
	DrawMonoImg(ImgSprites + xs + ys*IMGSPRITE_WBS, x, y, w, h, IMGSPRITE_WBS, col);
}

// invert sprite image
void DispSpriteInv(int xs, int ys, int x, int y, int w, int h)
{
	DrawMonoImgInv(ImgSprites + xs + ys*IMGSPRITE_WBS, x, y, w, h, IMGSPRITE_WBS);
}
...
void DispLives()
{
	...
	{
		if (i < Lives)
			DispSprite(LIVEIMGX, LIVEIMGY, i*LIVEDX, HEIGHT-LIVEH-1, LIVEW, LIVEH, SHIP_COL);
		else
			DrawRect(i*LIVEDX, HEIGHT-LIVEH-1, LIVEW, LIVEH, COL_BLACK);
	}
}
...
void DispScore()
{
	...
	buf[5] = 0;
	DrawTextBg(buf, SCOREX+4*8, SCOREY, COL_WHITE, COL_BLACK);
...
void DispLevel()
{
	...
	buf[2] = 0;
	DrawTextBg(buf, LEVELX+4*8, LEVELY, COL_WHITE, COL_BLACK);
}
...
void ShootHitHouse()
{
	if (ShootOn)
	{
		...
		{
			ShootOn = False;
			DrawRect(ShootX+1, HOUSEY, 2, HOUSEH, COL_BLACK);
		}
	}
}
...
void MissileHitHouse()
{
	...
	for (i = 0; i < MISSILENUM; i++)
	{
		if (MissileOn[i])
		{
			...
			{
				MissileOn[i] = False;
				DrawMonoImg(ImgSprites + MISSILEEXPIMGX + MISSILEEXPIMGY*IMGSPRITE_WBS,
					MissileX[i]-2, MissileY[i], MISSILEEXPW, MISSILEEXPH, IMGSPRITE_WBS, COL_BLACK);
			}
		}
	}
}
...
void GenMissile()
{
	...
	for (y = ALIENNUMY-1; y >= 0; y--)
	{
		for (x = ALIENNUMX-1; x >= 0; x--)
		{
			...
			{
				EnemyLast[x] = False;
				if ((RandU16() < 400) && (AlienY + y*ALIENDY + 8 < SHIPY))
				{
...
void NewGame(int level)
{
	...
	// display score
	DrawText("SCR:", SCOREX, SCOREY, COL_GREEN);
	DispScore();

	// display level
	Level = level;	// set current level
	DrawText("LVL:", LEVELX, LEVELY, COL_GREEN);
	DispLevel();
...
void Game()
{
	...
	while (True)
	{
		...
		if (AlienNum == 0)
		{
			DrawTextBg(" LEVEL UP ", (WIDTH-10*8)/2, (HEIGHT-8)/2, COL_RED, COL_YELLOW);
			DispUpdate();
			WaitMs(1000);
			KeyFlush();
			NewGame(Level+1);
		...
		if (Lives < 0)
		{
			DrawTextBg(" GAME OVER ", (WIDTH-11*8)/2, (HEIGHT-8)/2, COL_YELLOW, COL_RED);
			DispUpdate();
			WaitMs(500);
			StopSound();
			KeyFlush();
