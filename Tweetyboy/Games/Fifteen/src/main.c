...
#define TILEW		32	// tile width in pixels
#define TILEH		20	// tile height in lines
...
void DrawBoard()
{
	...
	for (i = 0; i < TILESYNUM; i++)
	{
		for (j = 0; j < TILESXNUM; j++)
		{
			...
			DrawImg(ImgTiles, x, y, 0, b*TILEH, TILEW, TILEH, 32);
		}
	}
	DispUpdate();
}
...
void WaitOn()
{
	...
	DrawCharBg(8, x+TILEW/2-4, y+6, COL_YELLOW, COL_BLACK);
	DispUpdate();
}
...
void WaitOn2()
{
	int x = HoleX()*TILEW + TILEX;
	int y = HoleY()*TILEH + TILEY;
	DrawCharBg(9, x+TILEW/2-4, y+6, COL_YELLOW, COL_BLACK);
	DispUpdate();
}
...
void WaitOff()
{
	...
	DrawImg(ImgTiles, x, y, 0, 15*TILEH, TILEW, TILEH, 32);
	DispUpdate();
}
