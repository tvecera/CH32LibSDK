...
#define BUTTON_DOWN KeyPressed(KEY_A) //(digitalRead(1) == 0)
#define BUTTON_UP (!KeyPressed(KEY_A)) //(digitalRead(1) == 1)
#define DEBOUNCE(ZELTADA) \
  _delay_ms(30); \
  if (ZELTADA)  // exemple:" if (BUTTON_DOWN) {DEBOUNCE(BUTTON_DOWN){ACTION(); } }   "
*/
...
uint8_t blitzSprite(int8_t xPos, int8_t yPos, uint8_t xPASS, uint8_t yPASS, uint8_t FRAME, const uint8_t *SPRITES)
{
	...
	if ((xPASS > ((xPos + (WSPRITE - 1)))) || (xPASS < xPos) ||
		((RECUPELINEY > yPASS) || ((RECUPELINEY + (HSPRITE)) < yPASS))) return 0x00;
...
uint8_t SPEED_BLITZ(int8_t xPos, int8_t yPos, uint8_t xPASS, uint8_t yPASS, uint8_t FRAME, const uint8_t *SPRITES)
{
	uint8_t WSPRITE = (pgm_read_byte(&SPRITES[0]));
	uint8_t HSPRITE = (pgm_read_byte(&SPRITES[1]));
	if ((xPASS > ((xPos + (WSPRITE - 1)))) || (xPASS < xPos) || ((yPASS < yPos)
		|| (yPASS > (yPos + (HSPRITE - 1))))) return 0x00;
	return pgm_read_byte(&SPRITES[2 + (((xPASS - xPos) + ((yPASS - yPos) *
		(WSPRITE))) + (FRAME * (HSPRITE * WSPRITE)))]);
}
