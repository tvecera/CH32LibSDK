...
NEW_START:
	if (KeyPressed(KEY_A)) goto NEW_START;
	...
	while(1)
	{
		...
		if (PLANTAGE_0==0)
		{
			...
			{
				// key left
				if (KeyPressed(KEY_LEFT) /*(analogRead(A0)>=750) && (analogRead(A0)<950)*/ && (ACCEL>1))
				{
					BypassWheelupreset=1;
					if ((Wheel_up<5) && (Latch1==0)) Wheel_up++;
				}

				// key right
				else if (KeyPressed(KEY_RIGHT) /*(analogRead(A0)>500) && (analogRead(A0)<750)*/ && (ACCEL>1))
				{
					BypassWheelupreset=1;
					if ((Wheel_up>0) && (Latch1==0))
					{
						Wheel_up--;
						if ((Wheel_up==0) && (FREEAIR==0)) Wheel_up=1;
					}
				}
				...
				if ((Not_Move==0) && (Not_Turn==0))
				{
					...
					// key up
					else if (KeyPressed(KEY_UP) /*(analogRead(A3)>500) && (analogRead(A3)<750)*/)
					{
						if (Trackrun_progress>0)
						{
							Trackrun_progress--;
							TRIG_OK=2;
						}
				...
				if ((TRIG_OK==0) && (Wheel_up==1) && (t>0)) 
				{
					animBike = (animBike==1) ? 6 : 1;
				}
			}

			if ((KeyPressed(KEY_A) && (ENDMAP!=1)) && ((FREEAIR==0) || (Wheel_up<=2)) && (End_game==0))
			{
				if (ACCEL<8) ACCEL=ACCEL+0.10;
			}
			else
			{
				if (ACCEL>1) ACCEL=ACCEL-0.10;
				BypassWheelupreset=0;
...
{
	if (Time_bar_Interval_timer<Time_bar_Interval)
	{
		Time_bar_Interval_timer++;
	}
	else
	{
		Time_bar_Interval_timer=0;
...
void GRAVITY_ADJ(void)
{
	...
	{
		if (BIKE_POSy<(PLANCHER-PLANCHER_add))
		{
			BIKE_POSy=BIKE_POSy+gravity_expo;
			gravity_expo=gravity_expo+0.2;
			FREEAIR=1;
		}
		else
		{
			FREEAIR=0;
			Not_Turn=0;
			BIKE_POSy=PLANCHER-PLANCHER_add;
			gravity_expo=0;
		}
	}
}
...
void JUMP_ADJ(void)
{
	if (Jump_dinamic_duration>=.24)
	{
		ADD_PILL=ADD_PILL+Jump_dinamic_duration;
		Jump_dinamic_duration=Jump_dinamic_duration-Higher_jump;
	}
	else
	{
		ADD_PILL=0;
	...
	{
		ADD_PILL=ADD_PILL-1;
		if (BIKE_POSy>0) BIKE_POSy=BIKE_POSy-1;
	}
}
...
#define yBike BIKE_POSy+12
	...
	for(t=0; t<NUMofSPRITE; t++)
	{
		...
		{
			if ((sprite[t].xPOS>xBike) ||
				((sprite[t].xPOS+RECUPE_X_SPRITE(sprite[NoSprite].TypeofSprite))<xBike) ||
				(trackrun>sprite[t].Y_END) ||
				(trackrun<sprite[t].Y_START))
				goto TIEND;
			NoSprite=t;
			goto Fin; 
TIEND:
...
void Break_Gravity(void)
{
	...
	{
		gravity_expo=0;
		if ((FREEAIR==0) && (Wheel_up==1))
		{
			if (ACCEL>5) ACCEL=ACCEL-2;
		}
	} 
}
...
void INCREMENTE_SCROLL(void)
{
	...
	{
		VAR_SCROLL3=2;
		if (Progress_bar_Interval_timer<Progress_bar_Interval)
		{
			Progress_bar_Interval_timer++;
		}
		else
		{
			Progress_bar_Interval_timer=0;
			if (Progress_bar_value<32) Progress_bar_value++;
...
uint8_t CREATE_NEWSPRITE(void)
{
	...
	for(t=0;t<NUMofSPRITE;t++)
	{
		if (sprite[t].ACTIVE==0)
		{
			...
			{
			case(0):
				sprite[t].TypeofSprite=1;
				sprite[t].yPOS=20;
				break;

			case(1):
				sprite[t].TypeofSprite=2;
				sprite[t].yPOS=26;
				break;

			case(2):
				sprite[t].TypeofSprite=3;
				sprite[t].yPOS=recupe_y_pos+32;
				break;

			case(3):
				sprite[t].TypeofSprite=4;
				sprite[t].yPOS=32;
				break;

			case(4):
				sprite[t].TypeofSprite=5;
				sprite[t].yPOS=32;
				break;

			case(5):
				sprite[t].TypeofSprite=6;
				sprite[t].yPOS=recupe_y_pos+22;
				break;

			case(6):
				sprite[t].TypeofSprite=7;
				sprite[t].yPOS=recupe_y_pos+34;
				break;

			case(7):
				sprite[t].TypeofSprite=8;
				sprite[t].yPOS=recupe_y_pos+30;
				break;

			default:
				sprite[t].TypeofSprite=16;
				break;
			}
			RENEW_SPRITE=0;
			if (type==14) ReWind=1;
...
{
	return ((0xff-pgm_read_byte(&DISPLAY8[xPASS]))
		| Return_live(xPASS)
		| Return_time(xPASS)
		| (Return_Progress(xPASS)));
}
...
uint8_t  BACKGROUND(uint8_t xPASS,uint8_t yPASS)
{
	switch(yPASS)
	{
	case(0): DispDirectDraw(xPASS,yPASS,FOUL1(),COL_CYAN/*color(21,52,31)*/); break;

	case(1):
	case(2): DispDirectDraw(xPASS,yPASS,GRADIN23(yPASS),COL_BLUE/*color(0,0,20)*/); break;

	case(3): DispDirectDraw(xPASS,yPASS,TIRE4(),COL_GREEN/*color(7,43,14)*/); break;

	case(4):
	case(5): DispDirectDraw(xPASS,yPASS,ROAD567(yPASS),COL_RED /*color(25,34,0)*/); break;

	case(6): DispDirectDraw(xPASS,yPASS,ROAD567(yPASS),COL_RED /*color(25,34,0)*/); break;

	case(7): DispDirectDraw(xPASS,yPASS,(0xff-TABLEAU8(xPASS)),COL_YELLOW /*color(31,52,19)*/); break;
...
uint8_t Recupe(uint8_t xPASS,uint8_t yPASS)
{
	...
	DispDirectDraw(xPASS,yPASS,BIKE_SPRITE(xPASS,yPASS),COL_WHITE /*color(31,63,31)*/);
	return 0;
}
...
uint8_t BLITZ_SPRITE_MAP(uint8_t xPASS,uint8_t yPASS)
{
	...
	for (t=0; t<NUMofSPRITE; t++)
	{
		if (sprite[t].ACTIVE!=0)
		{
			...
			{
			case(1): DispDirectDraw(xPASS,yPASS,BIGSTEP_SPRITE(t,xPASS,yPASS),COL_YELLOW/*color(25,34,0)*/); break;
			case(2): DispDirectDraw(xPASS,yPASS,MINISTEP_SPRITE(t,xPASS,yPASS),COL_YELLOW /*color(25,43,0)*/); break;
			case(3): DispDirectDraw(xPASS,yPASS,HUILE_SPRITE(t,xPASS,yPASS),COL_GREEN /*color(20,63,20)*/); break;
			case(4): DispDirectDraw(xPASS,yPASS,LINE_SPRITE(t,xPASS,yPASS),COL_GREEN /*color(0,40,20)*/); break;
			case(5): DispDirectDraw(xPASS,yPASS,START_SPRITE(t,xPASS,yPASS),COL_GREEN /*color(0,40,20)*/); break;
			case(6): DispDirectDraw(xPASS,yPASS,SPEED_SPRITE(t,xPASS,yPASS),COL_YELLOW/*color(31,31,0)*/); break;
			case(7): DispDirectDraw(xPASS,yPASS,Plantage(t,xPASS,yPASS),COL_WHITE /*color(31,63,31)*/); break;
			case(8): DispDirectDraw(xPASS,yPASS,NEW_LIVE(t,xPASS,yPASS),COL_CYAN /*color(0,63,31)*/); break;
			default:break;
			}
...
void Tiny_Flip(uint8_t MODE)
{
	...
	{ 
		...
		for (x = 0; x < 128; x++)
		{
			...
			case 2:
				DispDirectDraw(x,y,blitzSprite(38,30,x,y,0,INTROPIC),COL_CYAN /*color(0,63,31)*/);
				break;
