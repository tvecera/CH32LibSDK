...
u8 TileCharCol[TILE_NUM*2] = {
	' ',	COL_WHITE,	// S_BLACK = 0,		// 0: black tile
	' ',	COL_GRAY,	// S_EMPTY_HIDE,	// 1: hidden empty field (grass)
	' ',	COL_GRAY,	// S_WALL_HIDE,		// 2: hidden wall
	' ',	COL_GRAY,	// S_DOOR_HIDE,		// 3: hidden door
	1,	COL_GREEN,	// S_EMPTY,		// 4: empty field (grass)
	' '+128, COL_RED,	// S_WALL,		// 5: wall
	5,	COL_CYAN,	// S_DOOR,		// 6: door
	9,	COL_YELLOW,	// S_FACE,		// 7: face on empty field
};
...
void DrawBoard()
{
	...
	for (i = 0; i < MAPH; i++)
	{
		...
		{
			b = Board[i*MAPW + j];
			DrawCharCond6Bg(TileCharCol[b*2], j*TILESIZE+BOARDX, i*TILESIZE+BOARDY, TileCharCol[b*2+1], COL_BLACK);
		}
	}
	DispUpdate();
}
