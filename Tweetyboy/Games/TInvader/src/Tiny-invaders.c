...
//		_delay_ms(10);//Delay because is too fast with FastTinyDriver!
		...
		if (((space.MonsterGroupeXpos>=26) && (space.MonsterGroupeXpos<=28))
				&& (space.MonsterGroupeYpos==2) && (space.DecalageY8==4))
			space.UFOxPos=127;
		...
		if (ShipDead!=1)
		{
			...
			if (KeyPressed(KEY_RIGHT)) //(analogRead(A0)>500)&&(analogRead(A0)<750))
			{
				if (VarPot<108) VarPot=VarPot+6;
			}

			if (KeyPressed(KEY_A) /*(digitalRead(1)==0)*/ &&(MyShootReady==SHOOTS))
			{
				Sound(200,4);
				MyShootReady=0;
				space.MyShootBall=6;
...
const u8 TInvader_BgCol[8] = { COL8(5,2,3), COL_MAGENTA, COL8(7,0,2), COL_RED, COL8(7,5,0), COL_YELLOW, COL8(5,7,0), COL8(0,7,1) };
	...
	for (y = 0; y < 8; y++)
	{
		...
		for (x = 0; x < 128; x++)
		{
			if (render0_picture1==0)
			{
				...
				i2c_write(background(x,y,space) | LivePrint(x,y) | Vesso(x,y,space) |
					UFOWrite(x,y,space) | Monster(x,y,space) | MyShoot(x,y,space) |
					MonsterShoot(x,y,space) | MYSHIELD);
			}
			else
			{
				i2c_write(pgm_read_byte(&intro[x+(y*128)]));
			}
		}
...
void UFOUpdate(SPACE *space)
{
	...
	{
		space->UFOxPos=space->UFOxPos-2;SnD(space->UFOxPos,0);
		if (space->UFOxPos<=-20)
		{
			space->UFOxPos=-120;
		}
...
uint8_t Murge_Split_UP_DOWN(uint8_t x,SPACE *space)
{
	...
	{ //debut
		if (space->PositionDansGrilleMonsterY==0)
		{
			...
			if (SpriteType!=-1)
				Murge2=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos)+SpriteType*14)+ANIMs]),1,space);
			else
				Murge2=0x00;
			return Murge2;    
		}
		else
		{
			...
			if (SpriteType!=-1)
				Murge1=SplitSpriteDecalageY(pgm_read_byte(&Monsters[(WriteMonster14(x-space->MonsterGroupeXpos)+SpriteType*14)+ANIMs]),0,space);
			else
				Murge1=0x00;
