...
// tiles
#define TILEW	10	// tile width
#define TILEH	10	// tile height

// game board (size 80x80 pixels)
#define BOARDW	8	// game board width
#define BOARDH	8	// game board height
#define BOARDSIZE (BOARDW*BOARDH) // board size (= 64)
#define BOARDX	0
#define BOARDY	0

// frame
#define FRAMEW	80	// frame width
#define FRAMEH	80	// frame height
#define FRAMEX	80	// frame X coordinate
#define FRAMEY	0	// frame Y coordinate
...
#define TILE_ONMAX	TILE_HENWIN2 // last tile with cursor
...
void DrawTile(s8 x, s8 y)
{
	...
	if (b != DispBoard[y*BOARDW + x])
	{
		...
		DrawImg(ImgTiles, xx, yy, 0, b*TILEH, TILEW, TILEH, 10);
	}
}
...
void DispSel()
{
	...
	{
		if (play == Player)
			DrawFrame(FRAMEX, y, FRAMEW, 40, COL_RED);
		else
			DrawFrame(FRAMEX, y, FRAMEW, 40, COL_BLACK);
		y += FRAMEH/2;
	}
}
...
void DispScore()
{
	...
	for (play = 0; play < 2; play++)
	{
		int y = FRAMEY+play*(FRAMEH/2)+16;
		DrawRect(FRAMEX+2, y, FRAMEW-4, 8, COL_BLACK);
		n = DecNum(NumBuf, Stones[play], 0);
		DrawText(NumBuf, FRAMEX+4*8, y, COL_WHITE);
	}
}
...
void OpenGame(Bool comp1, Bool comp2)
{
	...
	// draw frame image
	DrawImg(ImgFrame, FRAMEX, FRAMEY, 0, 0, FRAMEW, FRAMEH, 80);
...
void OpenDraw()
{
	...
	// title
	DrawText("EGGS", (WIDTH-4*8)/2, 0*8, COL_YELLOW);

	// draw menu
	DrawText("Press key to start:", 0, 2*8, COL_GREEN);

	DrawText("LEFT .... Play eggs", 0, 4*8, COL_WHITE);
	DrawText("RIGHT ... Play hens", 0, 5*8, COL_WHITE);
	DrawText("DOWN .... 2 players", 0, 6*8, COL_WHITE);
	DrawText("UP ...... Demo", 0, 7*8, COL_WHITE);
	DrawText("Y ....... Quit", 0, 9*8, COL_WHITE);
