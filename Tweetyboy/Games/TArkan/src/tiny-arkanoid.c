...
const u8 MainCol[8] = { COL_WHITE, COL_WHITE, COL_YELLOW, COL_YELLOW,
	COL_CYAN, COL_CYAN, COL_BLUE, COL_BLUE };
...
#define VAl01 (VAR->LEVEL-(VAl10*10))
	if ((Y == 5) || (Y == 6)) Disp_OutCol = COL_YELLOW;
	if (Y==5)
...
u8 BlockCol[6] = { COL_GREEN, COL_BLUE, COL_YELLOW, COL_RED, COL_CYAN, COL_MAGENTA };
...
uint8_t Block(uint8_t X,uint8_t Y,GROUPE *VAR)
{
	...
	if ((X>=67)&&(X<97)&&(Y>=1)&&(Y<=6))
	{
		...
		Disp_OutCol = BlockCol[VAR->BlocsGrid[(Y-1)][XValue]];
