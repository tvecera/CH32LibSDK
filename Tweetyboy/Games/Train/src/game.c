...
void InfoClear()
{
	DrawRect(0, INFOY, WIDTH, 8, COL_BLACK);
}

// display text to info row
void InfoDispText(int x, const char* txt, u8 col)
{
	DrawTextCondBg(txt, x, INFOY, col, COL_BLACK);
}
...
{
	// display score
	InfoDispText(0, ScoreHlp, COL_GREEN); // 3*6=18 pixels title "SCORE"
	DrawRect(18, INFOY, 2, 8, COL_BLACK); // end on X = 20
	char buf[8];
	int n = DecUNum(buf, Score, 0); // 50 levels * 240 * 10 = 120000 max, 5 digits enough = 30 pixels
	InfoDispText(20, buf, COL_YELLOW); // end on X = 20+30 = 50

	// space
	DrawRect(20 + n*6, INFOY, 58 - 20 - n*6, 8, COL_BLACK);
	
	// display text A:PSW (30 pixels, end on X = 58+30 = 88)
	InfoDispText(58, LevelHlp, COL_RED);

	// space, start on X = 88, end on 116
	DrawRect(88, INFOY, WIDTH - 2*6 - 2 - 18 - 88, 8, COL_BLACK);

	// display level
	InfoDispText(WIDTH - 2*6 - 2 - 18, LvlTxt, COL_GREEN); // length "LVL" = 18 pixels, start on X=116, end on X=146
	DrawRect(WIDTH - 2*6 - 2, INFOY, 2, 8, COL_BLACK); // start on X=146, end on X=148
	n = DecUNum(buf, Level, 0);
	InfoDispText(WIDTH - 2*6, buf, COL_YELLOW); // start on X=148
	if (n == 1) DrawRect(WIDTH - 6, INFOY, 6, 8, COL_BLACK);
}
...
void Psw()
{
	...
	InfoDispText(WIDTH - (sizeof(EscHlp)-1)*6, EscHlp, COL_GREEN);
	...
	// display password
	int x = 50;
	InfoDispText(x, Psw2Txt, COL_RED);
	...
	for (;;)
	{
		...
		else
		{
			buf2[0] = buf[pos];
			PrintInv = 128;
			InfoDispText(x + pos*6, buf2, COL_YELLOW);
			PrintInv = 0;
		}
		DispUpdate();
...
void DispTile(u8 x, u8 y)
{
	...
	DrawImg(ImgTiles, x*TILEWIDTH + MAPX, y*TILEHEIGHT + MAPY,
		TileXY[tile*2]*TILEWIDTH, TileXY[tile*2+1]*TILEHEIGHT, TILEWIDTH, TILEHEIGHT, 160);
}
...
void CongLevel()
{
	...
	// set info box ON
	DrawRect(BOXX-3*6, BOXY, BOXW+6*6, BOXH, COL_BLACK);
	...
	// display text
	DrawTextCondBg(CongTxt, (WIDTH - (sizeof(CongTxt)-1)*6)/2, BOXY + 8, COL_YELLOW, COL_BLACK);
	DrawTextCondBg(Cong2Txt, (WIDTH - (sizeof(Cong2Txt)-1)*6)/2, BOXY + 2*8, COL_GREEN, COL_BLACK);
	DrawTextCondBg(Cong3Txt, (WIDTH - (sizeof(Cong3Txt)-1)*6)/2, BOXY + 3*8, COL_GREEN, COL_BLACK);
...
void OpenLevel()
{
	...
	DrawTextCondBg(LevelTxt, x, BOXY + 8, COL_GREEN, COL_BLACK);
	x += sizeof(LevelTxt)*6;
	DrawTextCondBg(buf, x, BOXY + 8, COL_YELLOW, COL_BLACK);

	// display password
	x = (WIDTH - (sizeof(PswTxt)+PSWLEN)*6)/2;
	DrawTextCondBg(PswTxt, x, BOXY + 3*8, COL_GREEN, COL_BLACK);
	memcpy(buf, &LevelPsw[Level*PSWLEN], PSWLEN);
	buf[PSWLEN] = 0;
	x += sizeof(PswTxt)*6;
	DrawTextCondBg(buf, x, BOXY + 3*8, COL_YELLOW, COL_BLACK);
...
void InitLevel()
{
	...
	int i, j;
	u8* dst = Board;
	const u8* src = &Levels[Level*MAPW*MAPH];
	for (i = 0; i < MAPH; i++)
	{
		for (j = 0; j < MAPW; j++)
		{
