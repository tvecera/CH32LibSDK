...
MENU:
	...
	{ 
		...
		if ((BUTTON_DOWN)&&(Ripple_filter_TTRIS==0))
		{
			PSEUDO_RND_TTRIS();
...
void CONTROLE_TTRIS(uint8_t *Rot_TTRIS)
{
	...
	if ((TINYJOYPAD_RIGHT==0)&&(TINYJOYPAD_LEFT==0))
	{
...
void Game_Play_TTRIS(void)
{
	...
	if (Level_TTRIS<21)
	{
		Level_Speed_ADJ_TTRIS=Mymap(Level_TTRIS,0,20,11,1);
	}
}
...
uint8_t blitzSprite_TTRIS(int8_t xPos,int8_t yPos,uint8_t xPASS,uint8_t yPASS,uint8_t FRAME,const uint8_t *SPRITES)
{
	...
	{
		OUTBYTE=SplitSpriteDecalageY_TTRIS(SPRITEyDECALAGE,pgm_read_byte(&SPRITES[ScanA+(PICBYTE)]),1);
	}
...
void Tiny_Flip_TTRIS(uint8_t HR_TTRIS)
{
	...
	{ 
		...
		{
			Disp_OutCol = ((x >= 44) && (x < 84)) ? COL_WHITE : COL_CYAN;
			i2c_write(Recupe_TTRIS(x,y));
		}
		i2c_stop();
	}
	DispUpdate();
...
void Flip_intro_TTRIS(uint8_t *TIMER1)
{
	...
	{ 
		...
		{
			Disp_OutCol = ((x >= 44) && (x < 84)) ? COL_WHITE : COL_CYAN;
			i2c_write(intro_TTRIS(x,y,TIMER1));
		}
		i2c_stop();
	}
	DispUpdate();
...
//SUM check
	for (t=0; t<Number_of_Backup; t++)
	{
		if ((checksum(Level_(t)))!=(Level_SUM[t]))
		{
			Level_SUM[t]=255;
			RESAVE=1;
		}
		...
		if ((checksum(Scores_A(t))+checksum(Scores_B(t)))!=(Scores_SUM[t]))
		{
			Scores_SUM[t]=255;
			RESAVE=1;
		}
