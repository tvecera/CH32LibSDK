...
#define DIGXL	((8*8-5)*COORD_MUL) // left score X
#define DIGXR	((10*8+4)*COORD_MUL) // right score X
...
void DispNet()
{
	DrawRect(((WIDTHM-NETW)/2)>>COORD_SHIFT, 0, NETW>>COORD_SHIFT, HEIGHT, COL_CYAN);
}
...
void DispScore(int inx)
{
	...
	// decode digits
	DrawMonoImgBg(ImgDigits + dig[0], x, 0, 8, 10, 10, COL_GREEN, COL_BLACK);
	DrawMonoImgBg(ImgDigits + dig[1], x+8, 0, 8, 10, 10, COL_GREEN, COL_BLACK);
}
...
void DispPaddle(int inx)
{
	...
	int y = PaddleY[inx];
	DrawRect(x, (y - PADDLEH/2)>>COORD_SHIFT, PADDLEW>>COORD_SHIFT, PADDLEH>>COORD_SHIFT, COL_YELLOW);
}
...
//		WaitMs(5);
		t = Time();
		TimeDelta = 3; //(t - OldTime)/(HCLK_PER_MS*17);
...
void OpenDraw()
{
	...
	// title
	DrawText("TV Tennis", (WIDTH-9*8)/2, 0*8, COL_YELLOW);

	// draw menu
	DrawText("Press key to start:", 0, 2*8, COL_GREEN);
	DrawText("LEFT....Left paddle", 0, 3*8, COL_WHITE);
	DrawText("RIGHT...Right paddle", 0, 4*8, COL_WHITE);
	DrawText("DOWN....2 players", 0, 5*8, COL_WHITE);
	DrawText("UP......Demo", 0, 6*8, COL_WHITE);
	DrawText("A.......Tied paddles", 0, 7*8, COL_WHITE);
