...
// Flash memory is organized to pages of size 256 bytes.
...
STATIC_ASSERT(sizeof(FLASH_t) == 0x28, "Incorrect FLASH_t!");
...
#define FLASH_GETCTLR ((FLASH->CTLR & ~(B6|B7|B15|B18|B19|B21|B22))|B9)

// @TODO: Use Flash_SckMod* with flash unlock to select Flash speed

// Set flash wait states 0..2 (register ACTLR.LATENCY)
// - Use 0 wait if sysclk <= 40MHz, 1 wait if sysclk > 40MHz and syclk <= 72MHz, 2 wait if sysclk > 72MHz
...
// Check if Flash write operation is busy (register STATR.WRBSY)
INLINE Bool Flash_WrBusy(void) { return (FLASH->STATR & B1) != 0; }
...
// Enable/Disable 4K sector erase mode (default disabled; register CTLR.PER)
...
INLINE void Flash_BlockEraseEnable(void) { FLASH->CTLR = FLASH_GETCTLR | B18; }
INLINE void Flash_BlockEraseDisable(void) { FLASH->CTLR = FLASH_GETCTLR & ~B18; }
...
// Start page programming (register CTLR.PGSTRT)
INLINE void Flash_PgStart(void) { FLASH->CTLR = FLASH_GETCTLR | B21; }

// Exit enhanced read mode (register CTLR.RSENACT)
INLINE void Flash_RsEnAct(void) { FLASH->CTLR = FLASH_GETCTLR | B22; }

// Enable/Disable enhnanced read mode (register CTLR.EHMOD)
INLINE void Flash_EhModEnable(void) { FLASH->CTLR = FLASH_GETCTLR | B24; }
INLINE void Flash_EhModDisable(void) { FLASH->CTLR = FLASH_GETCTLR & ~B24; }

// Set access clock to SysClk/1 or SysClk/2 - should be max. 60 MHz (register CTLR.SCKMOD)
INLINE void Flash_SckMod1(void) { FLASH->CTLR = FLASH_GETCTLR | B25; }
INLINE void Flash_SckMod2(void) { FLASH->CTLR = FLASH_GETCTLR & ~B25; }
...
// - Each bit represents 4K bytes (8 pages), 1=no write protection, 0=write protected
