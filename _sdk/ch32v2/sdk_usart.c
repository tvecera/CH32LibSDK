...
void USARTx_RCCClkEnable(USART_t* usart)
{
	...
	if (usart == USART3) USART3_RCCClkEnable();
	if (usart == USART4) USART4_RCCClkEnable();
	if (usart == USART5) USART5_RCCClkEnable();
	if (usart == USART6) USART6_RCCClkEnable();
	if (usart == USART7) USART7_RCCClkEnable();
	if (usart == USART8) USART8_RCCClkEnable();
}

// USART RCC clock Disable
void USARTx_RCCClkDisable(USART_t* usart)
{
	if (usart == USART1) USART1_RCCClkDisable();
	if (usart == USART2) USART2_RCCClkDisable();
	if (usart == USART3) USART3_RCCClkDisable();
	if (usart == USART4) USART4_RCCClkDisable();
	if (usart == USART5) USART5_RCCClkDisable();
	if (usart == USART6) USART6_RCCClkDisable();
	if (usart == USART7) USART7_RCCClkDisable();
	if (usart == USART8) USART8_RCCClkDisable();
}
...
void USARTx_Reset(USART_t* usart)
{
	...
	if (usart == USART3) USART3_Reset();
	if (usart == USART4) USART4_Reset();
	if (usart == USART5) USART5_Reset();
	if (usart == USART6) USART6_Reset();
	if (usart == USART7) USART7_Reset();
	if (usart == USART8) USART8_Reset();
}
...
void USARTx_Baud(USART_t* usart, int baud)
{
	...
	int clk = RCC_GetFreq((usart == USART1) ? CLK_PCLK2 : CLK_PCLK1);
