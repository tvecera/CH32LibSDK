...
//
// ****************************************************************************
// 12-bit conversion, max. 8MHz input clock, 14+1 channels (channel 15: reference voltage 1.2V)
// Conversion time Tconv = sanpling_time + 13Tadcclk
...
typedef struct {
	...
	io32	WDHTR;		// 0x24: watchdog 0 high threshold register
	io32	WDLTR;		// 0x28: watchdog 0 low threshold register
	...
	io32	CTLR3;		// 0x50: control register 3
	io32	WDTR1;		// 0x54: watchdog 1 threshold register
	io32	WDTR2;		// 0x58: watchdog 2 threshold register
	io32	WDTR3;		// 0x5C: watchdog 3 threshold register
} ADC_t;
STATIC_ASSERT(sizeof(ADC_t) == 0x60, "Incorrect ADC_t!");
...
#define ADC_JEXT_T2_CC2		4	// CC2 event of timer 2
#define ADC_JEXT_T3_CC2		5	// CC2 event of timer 3
#define ADC_JEXT_EXTI15		6	// EXTI line 15
...
#define ADC_EXT_T3_CC1		5	// CC1 event of timer 3
#define ADC_EXT_EXTI11		6	// EXTI line 11
...
// Select analog watchdog channel 0..15 (default 0; register CTLR1.AWDCH[3:0])
...
// Set data alignment to left to 16-bits (only when rule groups are used) or right (default right-aligned; register CTLR2.ALIGN)
...
// Set 12-bit data offset of injected channel 1/2/3/4 (default 0; register IOFR1/IOFR2/IOFR3/IOFR4)
...
// Set 12-bit watchdog 0 high threshold (default 0xfff; register WDHTR)
...
// Set analog watchdog 1..3 threshold low and high, 12-bit values (default 0x000 and 0xFFF; register WDTR1/WDTR2/WDTR3)
...
INLINE void ADCx_WD3(ADC_t* adc, int low, int high) { adc->WDTR3 = low | (high << 16); }

INLINE void ADC1_WD1(int low, int high) { ADCx_WD1(ADC1, low, high); }
INLINE void ADC1_WD2(int low, int high) { ADCx_WD2(ADC1, low, high); }
INLINE void ADC1_WD3(int low, int high) { ADCx_WD3(ADC1, low, high); }
...
// Select ADC channel chan=0..15 to be converted on the injected sequence seq=1..4 (default 0; register ISQR)
...
INLINE void ADCx_AwdScanEnable(ADC_t* adc) { adc->CTLR3 |= B9; }
INLINE void ADC1_AwdScanEnable(void) { ADCx_AwdScanEnable(ADC1); }

INLINE void ADCx_AwdScanDisable(ADC_t* adc) { adc->CTLR3 &= ~B9; }
INLINE void ADC1_AwdScanDisable(void) { ADCx_AwdScanDisable(ADC1); }

// Enable/Disable watchdog inx=0..3 output reset (default disable; register CTLR3.AWDx_RST_EN)
INLINE void ADCx_AwdRstEnable(ADC_t* adc, int inx) { adc->CTLR3 |= BIT(inx+12); }
INLINE void ADC1_AwdRstEnable(int inx) { ADCx_AwdRstEnable(ADC1, inx); }

INLINE void ADCx_AwdRstDisable(ADC_t* adc, int inx) { adc->CTLR3 &= ~BIT(inx+12); }
INLINE void ADC1_AwdRstDisable(int inx) { ADCx_AwdRstDisable(ADC1, inx); }

// Check/clear analog watchdog inx=0..3 comparison result - value is out of threshold values (register CTLR3.AWDx_RES)
INLINE Bool ADCx_AwdRes(ADC_t* adc, int inx) { return (adc->CTLR3 & BIT(inx + 16)) != 0; }
INLINE Bool ADC1_AwdRes(int inx) { return ADCx_AwdRes(ADC1, inx); }

INLINE void ADCx_AwdResClr(ADC_t* adc, int inx) { adc->CTLR3 &= ~BIT(inx + 16); }
...
// Get result of injection conversion channel 1..4, data right-aligned (register IDATAR1/IDATAR2/IDATAR3/IDATAR4)
