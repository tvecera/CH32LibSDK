...
// System clock source: 1=HSI, 2=HSE, 3=HSE_Bypass, 4=PLL_HSI, 5=PLL_HSE, 6=PLL_HSE_Bypass, 7=PLL_HSI/2, 8=PLL_HSE/2, 9=PLL_HSE_Bypass/2
	...
	// Set flash wait state  (0 wait if sysclk <= 40MHz, 1 wait if sysclk > 40MHz and syclk <= 72MHz, 2 wait if sysclk > 72MHz)
	Flash_Latency((RCC_FreqTab[CLK_HCLK] > 72*MHZ) ? 2 : ((RCC_FreqTab[CLK_HCLK] > 40*MHZ) ? 1 : 0));
}
...
void SystemInit(void)
{
	...
	mask =
		...
		RCC_APB1CLK_PWR |
#endif
#if !defined(ENABLE_LPTIM) || ENABLE_LPTIM	// LPTIM module enable
		RCC_APB1CLK_LPTIM |
#endif
		0;
