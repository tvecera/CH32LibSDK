...
#define IRQ_NUM		60		// number of interrupts
...
typedef struct {
	...
	io32	VTFBADDRR;		// 0x44: Fast interrupt service base address
	io32	CFGR;			// 0x48: Interrupt configuration register
	io32	GISR;			// 0x4C: (Read Only) Interrupt global status register
	io32	res[4];			// 0x50: ... reserved
	io32	VTFADDRR[4];		// 0x60: Fast interrupt offset address
...
#define EXTI_LINE19	19		// External interrupt line 19 Connected to the USBFS Wakeup event

// Hardware stack and nesting interrupt
#define NVIC_HWSTACKNEST_NO		(B0|B1)	// hardware stack OFF, nesting interrupt OFF
#define NVIC_HWSTACKNEST_HWSTACK	B1	// hardware stack ON, nesting interrupt OFF
#define NVIC_HWSTACKNEST_NEST		B0	// hardware stack OFF, nesting interrupt ON
#define NVIC_HWSTACKNEST_ALL		0	// hardware stack ON, nesting interrupt ON
...
// - Vector table must be located in RAM, or use weak handler otherwise, or use fast interrupt service channel.
...
{
	u32 t = NVIC->ITHRESDR; // save current interrupt priority threshold value
	NVIC->ITHRESDR = 16;	// redefine interrupt priority threshold, to not execute interrupts with priority >= 16
	NVIC->IRER[irq >> 5] = BIT(irq & 0x1f);
	NVIC->ITHRESDR = t;	// restore interrupt priority threshold value
}
...
INLINE void sev(void) { NVIC->SCTLR |= B3|B5; }
...
// Setup enable hardware stack and nesting interrupt NVIC_HWSTACKNEST_*
INLINE void NVIC_HWStackNest(int val) { NVIC->CFGR = NVIC_KEY1 | val; }

// NMI interrupt suspension
INLINE void NVIC_NMISet(void) { NVIC->CFGR = NVIC_KEY2 | B2; }

// NMI interrupt reset pending
INLINE void NVIC_NMIClr(void) { NVIC->CFGR = NVIC_KEY2 | B3; }

// Exception interrupt suspension
INLINE void NVIC_EXCSet(void) { NVIC->CFGR = NVIC_KEY2 | B4; }

// Exception interrupt reset pending
INLINE void NVIC_EXCClr(void) { NVIC->CFGR = NVIC_KEY2 | B5; }

// PFIC (NVIC) module reset
INLINE void NVIC_Reset(void) { NVIC->CFGR = NVIC_KEY2 | B6; }
