...
// Flash memory is organized to 512 pages of size 128 bytes (total 64 KB).
...
// - Use 0 wait if sysclk <= 24MHz, 1 wait if sysclk > 24MHz and syclk <= 48MHz, 2 wait if sysclk > 48MHz
...
// Flash pages
#define FLASH_PAGEER_SHIFT	7			// shift of page erase size
#define FLASH_PAGEER_SIZE	BIT(FLASH_PAGEER_SHIFT)	// size of page erase (128 B)
#define FLASH_PAGEPG_SHIFT	7			// shift of page program size
#define FLASH_PAGEPG_SIZE	BIT(FLASH_PAGEPG_SHIFT)	// size of page program (128 B)
...
#define FLASH_GETSTATR ((FLASH->STATR & ~(B2|B4|B5))|B6)
...
// Lock option-byte programming (register CTLR.OBWRE, CTLR.OPTWRE)
...
// Check/Clear program error flag (register STATR.WRPRTERR)
INLINE Bool Flash_PGErr(void) { return (FLASH->STATR & B2) != 0; }
INLINE Bool Flash_PGErrClr(void) { FLASH->STATR = FLASH_GETSTATR | B2; }
...
// Erase flash memory range, with timeout in [ms] per 128 B page (address and size must be multiply of 128 B = 0x80; returns False on error)
