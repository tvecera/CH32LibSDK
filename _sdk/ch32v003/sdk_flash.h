...
// Flash memory is organized to 256 pages of size 64 bytes (total 16 KB).
...
// Set flash wait states 0..1 (register ACTLR.LATENCY)
// - Use 0 wait if sysclk <= 24MHz, 1 wait if sysclk >= 24MHz
...
typedef struct {
	...
	io16	USER;			// 0x02: bit 0: IWDG_SW 1=is opened by software (default), 0=turned on by hardware
					//	 bit 1: STANDYRST system reset in standby mode 1=not enabled (default), 0=enabled
...
// Flash pages
#define FLASH_PAGEER_SHIFT	6			// shift of page erase size
#define FLASH_PAGEER_SIZE	BIT(FLASH_PAGEER_SHIFT)	// size of page erase (64 B)
#define FLASH_PAGEPG_SHIFT	6			// shift of page program size
#define FLASH_PAGEPG_SIZE	BIT(FLASH_PAGEPG_SHIFT)	// size of page program (64 B)
...
// - Each bit represents 1K bytes (16 pages), 1=no write protection, 0=write protected
...
// Erase flash memory range, with timeout in [ms] per 64 B page (address and size must be multiply of 64 B = 0x40; returns False on error)
