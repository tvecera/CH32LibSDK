...
//
// ****************************************************************************
// TIM3 is available only for CH32V06, CH32V007 and CH32M007
...
// TIM3 streamlined timer registers (only CH32V06, CH32V007 and CH32M007)
typedef struct {
	io32	CTLR1;		// 0x00: control register
	io32	DMAINTENR;	// 0x04: DMA/interrupt enable register
	io32	CNT;		// 0x08: counter
	io32	ATRLR;		// 0x0C: auto-reload value register
	io32	CH1CVR;		// 0x10: compare/capture register 1
	io32	CH2CVR;		// 0x14: compare/capture register 2
	io32	CH3CVR;		// 0x18: compare/capture register 3
	io32	CH4CVR;		// 0x1C: compare/capture register 4
} TIM3_t;
STATIC_ASSERT(sizeof(TIM3_t) == 0x20, "Incorrect TIM3_t!");
#define TIM3	((TIM3_t*)TIM3_BASE)	// 0x40000800
...
// === TIM3 Control (TIM3 is available only for CH32V06, CH32V007 and CH32M007)

// Enable/Disable counter (default disabled; register CTLR1.CEN)
INLINE void TIM3_Enable(void) { TIM3->CTLR1 |= B0; }
INLINE void TIM3_Disable(void) { TIM3->CTLR1 &= ~B0; }

// Enable/Disable generation of UEV update events (default enabled; register CTLR1.UDIS)
INLINE void TIM3_UEVEnable(void) { TIM3->CTLR1 &= ~B1; }
INLINE void TIM3_UEVDisable(void) { TIM3->CTLR1 |= B1; }

// Select counting direction up or down (default up direction; register CTLR1.DIR)
INLINE void TIM3_DirUp(void) { TIM3->CTLR1 &= ~B4; }
INLINE void TIM3_DirDown(void) { TIM3->CTLR1 |= B4; }

// Set alignment mode selection TIM_ALIGN_* (default edge alignment; register CTLR1.CMS[1:0])
INLINE void TIM3_Align(int align) { TIM3->CTLR1 = (TIM3->CTLR1 & ~(3<<5)) | (align<<5); }

// Enable/Disable auto-reload (default disabled; register CTLR1.ARPE)
INLINE void TIM3_AutoReloadEnable(void) { TIM3->CTLR1 |= B7; }
INLINE void TIM3_AutoReloadDisable(void) { TIM3->CTLR1 &= ~B7; }

// Select input mode TIM_INMODE_*, not encoder mode (default internal clock CK_INT; register CTLR1.SMS)
INLINE void TIM3_InMode(int inmode) { TIM3->CTLR1 = (TIM3->CTLR1 & ~(7<<8)) | (inmode<<8); }

// Compare mode: Enable/Disable preload compare register 1 (default disable; register DMAINTENR.OC1PE)
INLINE void TIM3_OC1PreEnable(void) { TIM3->DMAINTENR |= B0; }
INLINE void TIM3_OC1PreDisable(void) { TIM3->DMAINTENR &= ~B0; }

// Compare mode: Enable/Disable preload compare register 2 (default disable; register DMAINTENR.OC2PE)
INLINE void TIM3_OC2PreEnable(void) { TIM3->DMAINTENR |= B1; }
INLINE void TIM3_OC2PreDisable(void) { TIM3->DMAINTENR &= ~B1; }

// Compare mode: Enable/Disable preload compare register 3 (default disable; register DMAINTENR.OC3PE)
INLINE void TIM3_OC3PreEnable(void) { TIM3->DMAINTENR |= B2; }
INLINE void TIM3_OC3PreDisable(void) { TIM3->DMAINTENR &= ~B2; }

// Compare mode: Enable/Disable preload compare register 4 (default disable; register DMAINTENR.OC4PE)
INLINE void TIM3_OC4PreEnable(void) { TIM3->DMAINTENR |= B4; }
INLINE void TIM3_OC4PreDisable(void) { TIM3->DMAINTENR &= ~B4; }

// Enable/Disable compare channel 3 DMA request (default disabled; register DMAINTENR.CC3DE)
INLINE void TIM3_CC3DMAReqEnable(void) { TIM3->DMAINTENR |= B11; }
INLINE void TIM3_CC3DMAReqDisable(void) { TIM3->DMAINTENR &= ~B11; }

// Enable/Disable compare channel 4 DMA request (default disabled; register DMAINTENR.CC4DE)
INLINE void TIM3_CC4DMAReqEnable(void) { TIM3->DMAINTENR |= B12; }
INLINE void TIM3_CC4DMAReqDisable(void) { TIM3->DMAINTENR &= ~B12; }

// Set/Get counter 0..65535 (default 0; register CNT)
INLINE void TIM3_Cnt(int cnt) { TIM3->CNT = cnt; }
INLINE u16 TIM3_GetCnt(void) { return (u16)TIM3->CNT; }

// Set/Get auto-reload value 0..65535 (default 0; register ATRLR)
INLINE void TIM3_Load(int val) { TIM3->ATRLR = val; }
INLINE u16 TIM3_GetLoad(void) { return (u16)TIM3->ATRLR; }

// Set/Get compare value of channel 1..4, 0..65535 (default 0; register CH1CVR, CH2CVR, CH3CVR, CH4CVR)
INLINE void TIM3_Comp1(int val) { TIM3->CH1CVR = val; }
INLINE u16 TIM3_GetComp1(void) { return (u16)TIM3->CH1CVR; }
INLINE void TIM3_Comp2(int val) { TIM3->CH2CVR = val; }
INLINE u16 TIM3_GetComp2(void) { return (u16)TIM3->CH2CVR; }
INLINE void TIM3_Comp3(int val) { TIM3->CH3CVR = val; }
INLINE u16 TIM3_GetComp3(void) { return (u16)TIM3->CH3CVR; }
INLINE void TIM3_Comp4(int val) { TIM3->CH4CVR = val; }
INLINE u16 TIM3_GetComp4(void) { return (u16)TIM3->CH4CVR; }
